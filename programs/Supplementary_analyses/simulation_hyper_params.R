...
eval(parse(text=args[2])) # these assignes a value to the hyper-parameters of the piecewise exponentail conjugate gamma priors 
                          # u = shape parameter, v= gamma parameter
                          # The script must be called as follows:
                          # R CMD BATCH '--args u=<value> v=<values>' <filename.R> 
...
sim.results <- function(NPERM=1e3,KM0,KM1,KMCens,NPHASE2,NPHASE3,phase3data,u,v){
  # New phase 2 study 
  ...
  post <- get.post(times  = phase2$Time,
                   arms   = phase2$Arm,
                   events = phase2$Event,
                   alpha.0 = rep(u,times=5),
                   beta.0  = rep(v,times=5),
                   alpha.1 = rep(u,times=5),
                   beta.1  = rep(v,times=5))
  breaks       <- post$breaks
  ...
  trial <- sim.trial.KM(NPHASE3,KM0,KM1,KMCens,phase3data)
  times <- trial$Time
  arm   <- trial$Arm
  event <- trial$Event
...
# run the simulation
sims <- sapply(1:NSIMS, FUN=function(i) 
  sim.results(NPERM,KM0,KM1,KMCens,NPHASE2,NPHASE3,phase3, u, v))

# save the simulation results and the simulation sample sizes
outname <- paste0("./datasets/Supplementary_simulations/",
                  "sim_hyper_u",
                  u,"_v",v,
                  ".Rdata")
out <- list(sims=sims, u=u, v=v)
save(list=c("out"),file=outname)
