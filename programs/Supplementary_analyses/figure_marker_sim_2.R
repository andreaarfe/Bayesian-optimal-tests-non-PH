...
library(ggplot2)
library(tidyverse)
load("./datasets/Supplementary_simulations/phase3_marker_sim_2.Rdata")
...
p <- d %>% 
	...
	scale_x_continuous(breaks = c(0.20,0.80),minor_breaks = c(0,0.4,0.6,1), limits=c(0,1))

ggsave("./results/Supplementary_results/figure_marker_2.pdf",
			 width=10,height=4,plot=p,device = "pdf")
